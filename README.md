...
- **频道组织**：自动按 `group-title` 分组，任意调整分组顺序、频道置顶/隐藏，收藏夹独立入口
...
JAVA_HOME=<jdk17> gradle :app:testReleaseUnitTest # 单元测试(Robolectric 16 项)
```
...
app/src/main/java/com/rhodes/tv/
  MainActivity.kt        主界面(分组/频道列表、遥控器按键)
  PlayerActivity.kt      播放页
  PlayerController.kt    播放控制/缓冲策略
  SettingsActivity.kt    设置页(订阅源、更新、端口、偏好)
  ConfigServer.kt        手机远程管理 HTTP 服务
  Updater.kt             应用内一键更新(多镜像/校验/安装)
  ChannelStore.kt        频道解析与存储
  Prefs.kt / EpgStore.kt / ...
